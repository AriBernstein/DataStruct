...
namespace Fractional_Cascading {
    ...
    public class BinarySearchNodes {
        ...
        // TODO - make a damn subclass
        public int binarySearchCoordNode(CoordinateNode[] nodeArray, int searchValue, int attrCode) {
            return binarySearchCoordNode(nodeArray, 0, nodeArray.Length-1, searchValue, attrCode);
        }
        private int binarySearchCoordNode(CoordinateNode[] nodeArray,
                                          int l, int r, int data, int attrCode) {
            // Base case
            String xNotInArrException =
                "data: " + data + " cannot be found in FC Node array during binary search";                                                  
            ...
            // Data less than element at mid, search left side
            else if(data < dataAtMid) return binarySearchCoordNode(nodeArray, l, m-1, data, attrCode);

            // Data greater than element at mid, search right side
            else return binarySearchCoordNode(nodeArray, m+1, r, data, attrCode);
        }
